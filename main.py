...
def download(
    ...
    if type == "audio":
        ...
        # Falls back to a combined best format if the specific one disappears mid-request.
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail="Download failed. Try a lower quality or another link.",
        )

    matches = glob.glob(os.path.join(TMP_DIR, f"{job_id}.{final_ext}"))
    if not matches:
        # yt-dlp sometimes keeps the source extension when no conversion was needed
...
# Serve the single-page frontend. Must be mounted last so /api/* routes above take priority.
